        ...
        const periodStr = analysisPeriod === 'week'
            ...
            : analysisPeriod === 'year'
                ...
                : analysisPeriod === 'month'
                    ...
                    : 'Custom range';
        subtitleEl.innerText = `${filtered.length} transaction${filtered.length !== 1 ? 's' : ''} Â· ${periodStr}`;
    }
    ...
    categories.forEach(cat => {
        ...
        catAccs.forEach(acc => {
            ...
            if (acc.type === 'card') {
                ...
                // Respect Cash Wallet exclusion preference
                if (acc.type === 'cash' && !includeCashInBalance) return;
...
function updateBudget() {
    ...
    if (headerDateText) {
        if (activeBudgetTab === 'monthly') {
            headerDateText.innerText = `${monthNames[currentMonth].slice(0, 3)} ${String(currentYear).slice(-2)} â€¢ Summary`;
        } else {
            headerDateText.innerText = `${currentYear} â€¢ Annual Summary`;
        }
    }
    ...
    userAccounts.forEach(acc => {
        ...
        btn.innerHTML = `
            ...
            <div class="flex-grow text-left min-w-0">
                ...
                <span class="block text-label-md text-on-surface-variant truncate">${acc.holderName ? acc.holderName + ' Â· ' : ''}${acc.type === 'bank' ? 'Bank Account' : acc.type === 'card' ? 'Credit Card' : 'Cash'}</span>
            </div>
        `;
...
function deleteTag(tag) {
    ...
    if (tagDropdown) {
        const tags = new Set();
        transactions.forEach(t => { if (t.tags) t.tags.forEach(tg => tags.add(tg)); });
...
function renderIncomeTransactions(loadMore = false) {
    ...
    const incomeList = transactions.filter(t => t.type === 'income' && transactionBelongsToMonth(t, incomeDetailsMonth, incomeDetailsYear) && matchTransaction(t, searchVal));
...
function renderSpendingTransactions(loadMore = false) {
    ...
    const expenseList = transactions.filter(t => t.type === 'expense' && transactionBelongsToMonth(t, spendingDetailsMonth, spendingDetailsYear) && matchTransaction(t, searchVal));
...
function renderPickerCalendar() {
    ...
    grid.addEventListener('touchend', (e) => {
        ...
        if (Math.abs(diff) > 40) changePickerMonth(diff > 0 ? 1 : -1);
    }, { passive: true });
...
function updateCalcDisplay() {
    ...
    if (expr) {
        expr.innerText = calcExpression.replace(/\*/g, 'Ã—').replace(/\//g, 'Ã·');
    }
}
...
// Unsaved changes state tracking and protection dialog logic
...
function unsavedDialogSave() {
    dismissUnsavedDialog();
    if (currentFormContext && currentFormContext.saveFn) {
