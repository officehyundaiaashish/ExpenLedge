...
function renderStructuredTx(loadMore = false) {
    ...
    const txCountEl = document.getElementById('structured-tx-count');
    {
        ...
        if (totalBalanceEl) {
            ...
            if (balCard) {
                balCard.className = `px-4 py-1.5 rounded-full ${bal >= 0 ? 'bg-white/20 border border-white/20 text-white font-bold' : 'bg-red-500/20 border border-red-500/30 text-red-100 font-bold'}`;
            }
        }
    }
    ...
    for (const groupName in groups) {
        ...
        groups[groupName].forEach(t => {
            ...
            itemEl.className = "p-md rounded-xl flex items-center gap-md hover:bg-surface-container-high transition-all cursor-pointer active:scale-[0.98]";
