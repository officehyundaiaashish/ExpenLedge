...
function unsavedDialogSave() {
    ...
    const savedContext = currentFormContext;
    currentFormContext = null;
    savedContext.saveFn();
    // Map each form name to its sheet element ID
    const formSheetMap = {
        'transaction': 'modal-add-transaction',
        'profile':     'sheet-profile-security',
        'budget':      'sheet-edit-budget',
        'account':     'sheet-edit-account',
        'category':    'sheet-add-category'
    };
    const sheetId = formSheetMap[savedContext.name];
    const sheetEl = sheetId ? document.getElementById(sheetId) : null;
    // If sheet is still open (save was rejected by validation), restore context
    // so the unsaved-changes dialog works again on the next close attempt
    if (sheetEl && !sheetEl.classList.contains('translate-y-full')) {
        setTimeout(() => {
            if (!currentFormContext) {
                currentFormContext = {
                    name: savedContext.name,
                    snapshot: snapshotFormState(savedContext.name),
                    saveFn: savedContext.saveFn,
                    closeFn: savedContext.closeFn
                };
            }
        }, 60);
    }
