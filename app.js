...
function renderIncomeTransactions(loadMore = false) {
    ...
    if (incomeTotalEl) incomeTotalEl.textContent = `₹${incomeTotal.toFixed(2)}`;
...
function renderSpendingTransactions(loadMore = false) {
    ...
    const spendingTotal = expenseList.reduce((sum, t) => sum + t.amount, 0);
    const spendingTotalEl = document.getElementById('spending-sheet-total');
    if (spendingTotalEl) spendingTotalEl.textContent = `₹${spendingTotal.toFixed(2)}`;
...
function unsavedDialogSave() {
    dismissUnsavedDialog();
    const saveFn = currentFormContext && currentFormContext.saveFn;
    currentFormContext = null;
    if (saveFn) saveFn();
}
