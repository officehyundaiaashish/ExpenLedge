...
function renderStructuredTx(loadMore = false) {
    ...
    const txCountEl = document.getElementById('structured-tx-count');
    {
        ...
        // Update card period label
        const periodTypeLabelEl = document.getElementById('structured-period-type-label');
        if (periodTypeLabelEl) {
            if (structuredTxMode === 'day') periodTypeLabelEl.innerText = 'NET THIS DAY';
            else if (structuredTxMode === 'month') periodTypeLabelEl.innerText = 'NET THIS MONTH';
            else periodTypeLabelEl.innerText = 'NET THIS RANGE';
        }
    }
