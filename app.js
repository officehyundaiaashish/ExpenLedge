...
function suppressBrowserAutofill(root = document) {
    ...
    fields.forEach((el) => {
        ...
        if (!el.dataset.autofillGuardName) {
            el.dataset.autofillGuardName = `expenledge_${Math.random().toString(36).slice(2)}`;
            el.setAttribute('name', el.dataset.autofillGuardName);
        }
...
function saveInterfacePreferences() {
    localStorage.setItem('expenledge_interface_preferences', JSON.stringify({
        ...
        analysisYear,
        analysisMonth,
        analysisWeekStart: analysisWeekStart.toISOString(),
        analysisCatType,
        allTxSort,
        analysisCustomFrom: document.getElementById('analysis-custom-from')?.value || '',
        analysisCustomTo: document.getElementById('analysis-custom-to')?.value || ''
    }));
}
...
function loadInterfacePreferences() {
    ...
    if (['week', 'month', 'year', 'custom'].includes(preferences.analysisPeriod)) analysisPeriod = preferences.analysisPeriod;
    if (Number.isInteger(preferences.analysisYear)) analysisYear = preferences.analysisYear;
    if (Number.isInteger(preferences.analysisMonth) && preferences.analysisMonth >= 0 && preferences.analysisMonth <= 11) analysisMonth = preferences.analysisMonth;
    if (preferences.analysisWeekStart) {
        const weekStart = new Date(preferences.analysisWeekStart);
        if (!Number.isNaN(weekStart.getTime())) analysisWeekStart = weekStart;
    }
    if (['spending', 'income'].includes(preferences.analysisCatType)) analysisCatType = preferences.analysisCatType;
    if (['date-desc', 'date-asc', 'amount-desc', 'amount-asc'].includes(preferences.allTxSort)) allTxSort = preferences.allTxSort;

    const customFrom = document.getElementById('analysis-custom-from');
    const customTo = document.getElementById('analysis-custom-to');
    if (customFrom && preferences.analysisCustomFrom) customFrom.value = preferences.analysisCustomFrom;
    if (customTo && preferences.analysisCustomTo) customTo.value = preferences.analysisCustomTo;
}
