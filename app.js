...
async function syncSupabaseNow(options) {
    ...
    try {
        ...
        // Always do the cheap metadata-only check first (a few hundred bytes),
        // whether this is an auto-sync or a manual "Sync Now" tap. We only
        // pay for the full payload pull (5-10MB for 10k+ transactions) when
        // the remote `updated_at` actually differs from what we last applied.
        if (lastRemoteApplied) {
        ...
        if (currentFingerprint === lastPushFp) {
            // Nothing to push and (per step 1) nothing new to pull → done.
            // This now applies to manual "Sync Now" too — no reason to
            // re-upload the entire 10k+ transaction payload if nothing
        ...
        // Version-history table is intentionally NOT written here anymore.
        // It used to insert a second full copy of the entire payload into
        // `expenledge_state_versions` on every single sync, which doubled
        // the amount of data uploaded each time for no user-visible benefit.
        // Only `expenledge_state` (one row per device) is kept in sync now.
