    ...
    if (badge) {
        if (isConnected) {
            ...
            if (!document.getElementById('supabase-sync-keyframes')) {
                ...
                style.textContent = '@keyframes supabase-sync-pop { 0% { transform: scale(1); opacity: 0.5; } 40% { transform: scale(1.25); opacity: 1; } 70% { transform: scale(0.95); } 100% { transform: scale(1); opacity: 1; } }';
...
function animateFabOnSync(result) {
    if (!document.getElementById('fab-sync-keyframes')) {
        ...
        style.textContent = [
            '@keyframes fab-rotate-left { 0% { transform: rotate(0deg); } 100% { transform: rotate(-360deg); } }',
            '@keyframes fab-rotate-right { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }',
    ...
    const fabIcon = document.querySelector('#global-structured-fab button span.material-symbols-outlined');

    [fabIcon].filter(Boolean).forEach(el => {
        el.style.animation = 'none';
        void el.offsetWidth;
        if (result === 'success') {
            el.style.animation = 'fab-rotate-left 0.6s ease-in-out forwards';
        } else if (result === 'fail') {
            el.style.animation = 'fab-rotate-right 0.6s ease-in-out forwards';
        } else if (result === 'error') {
            el.style.animation = 'fab-nod 0.7s ease-in-out forwards';
        }
    });
}
        ...
        const periodStr = analysisPeriod === 'week'
            ...
            : analysisPeriod === 'year'
                ...
                : analysisPeriod === 'month'
                    ...
                    : 'Custom range';
        subtitleEl.innerText = `${filtered.length} transaction${filtered.length !=